...
class Model extends DB
{
    ...
    // {
    //     $select = mysqli_query($this->connect(), "SELECT * FROM registered WHERE username = '$username' AND role='manager' AND status= 'active' AND password = '$password'");
    //     return $select;
    // }
    protected function userLogin($username, $password)
    {
        ...
        if ($result->num_rows > 0) {
            ...
            $_SESSION["managerlastname"] = $row['lastname'];
    ...
    public function showInvoiceManager()
    {
        $date = date("d-m-Y");
        $select = mysqli_query($this->connect(), "SELECT * FROM sales WHERE staff_name = '" . $_SESSION['managerfullname'] . "' AND date LIKE '%$date%' ORDER BY id DESC");
        return $select;
    }
    ...
    public function addCode($code){
        mysqli_query($this->connect(), "INSERT INTO code VALUES(null,'$code')");
