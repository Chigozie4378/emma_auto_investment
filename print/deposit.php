...
// Simple fallback if session is not ready
if (empty($_SESSION["invoice_no_deposit"]) || empty($_SESSION["active_role"])) {
...
$result = $deposit_ctr->checkDepositByInvoiceNo($_SESSION["invoice_no_deposit"]);
...
$select_pos = $sales_history_ctr->showPosByInvoice_no($_SESSION["invoice_no_deposit"]);
$result_pos = mysqli_fetch_assoc($select_pos);
$select = $deposit_ctr->checkDepositDetailsByInvoiceNo($_SESSION['invoice_no_deposit']);
