...
<script>
    function record(date) {
        ...
        fetch("/emma_auto_investment/ajax/admin/sales_record/load_sale_record.php?date=" + encodeURIComponent(date))
            .then(res => res.text())
            .then(html => {
    ...
    function findInvoice(invoice_no) {
        ...
        fetch("/emma_auto_investment/ajax/admin/sales_record/load_sales_invoice.php?invoice_no=" + encodeURIComponent(invoice_no))
            .then(res => res.text())
            .then(html => {
                document.getElementById("table").innerHTML = html;
                window.bindRowClicks(); 
    ...
    function findStaff(date, staff) {
        ...
        fetch("/emma_auto_investment/ajax/admin/sales_record/load_sales_staff.php?date=" + encodeURIComponent(date) + "&staff=" + encodeURIComponent(staff))
            .then(res => res.text())
            .then(html => {
                document.getElementById("table").innerHTML = html;
                window.bindRowClicks(); 
